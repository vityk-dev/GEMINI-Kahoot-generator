...
if api_key:
        if st.button("Generate Quiz"):
            ...
            # 1. Tworzymy Klienta (zamiast genai.configure)
            client = genai.Client(api_key=api_key)
            
            prompt = f"""
            ... (Twój prompt pozostaje bez zmian) ...
            """
            
            with st.spinner("Generating quiz..."):
                try:
                    # 2. Nowy sposób wywoływania modelu przez klienta
                    # Zauważ: 'model' podajemy jako argument, a nie tworzymy obiektu
                    ...
                    # ... (Reszta kodu parsującego JSON pozostaje bez zmian) ...
                    # Clean the response...
                    json_response = response.text.strip().replace("```json", "").replace("```", "")
                    quiz_data = json.loads(json_response)
                    
                    # ... (Reszta Twojego kodu do wyświetlania i pobierania) ...

                except Exception as e:
                    st.error(f"An error occurred: {e}")
else:
    available_models = ['models/gemini-1.5-pro', 'models/gemini-1.5-flash'] # Fallback

# Remove "models/" prefix for display
display_models = [model.replace("models/", "") for model in available_models]

# Set a default model based on availability or preference
default_model_index = 0
if 'gemini-1.5-flash' in display_models:
    default_model_index = display_models.index('gemini-1.5-flash')
elif 'gemini-1.5-pro' in display_models:
    default_model_index = display_models.index('gemini-1.5-pro')
...
# Add back "models/" prefix for API call
...
if uploaded_file is not None:
    ...
    if api_key: # Check if API key is provided in the input field
        if st.button("Generate Quiz"):
            # Initialize the client (not strictly needed here for generate_content, but good for context)
            # client = genai.Client(api_key=api_key) # This line is actually not needed here if model is initialized with API key
            genai.configure(api_key=api_key) # Configure the API key globally for genai.GenerativeModel
